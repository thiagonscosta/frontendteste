<template>
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-6">
                <h2 class="mt-3 text-muted">Populares</h2>
            </div>
            <div class="col-md-6">
                <movies-filter />
            </div>
...
export default {
    ...
    created: function() {
        ...
        this.$eventBus.$on('selected_popular_movie_title', function (title) {
            vm.filterMoviesByTitle(title)
        })

        this.$eventBus.$on('selected_popular_movie_genre', function (genre_id) {
            vm.filterMoviesByGenre(genre_id)
        })

        this.$eventBus.$on('selected_year', function (year) {
            vm.filterMoviesByGenre(year)
        })
    },
    ...
    methods: {
        fetchMovies (p) {
            let data = [{}]
            axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.VUE_APP_API_KEY}&language=pt-BR&page=${p}`)
            .then(response => {
        ...
        loadMovies (data) {
            ...
            if (this.p < this.total_pages) {
                ...
                this.displayMovies()
                this.sendPopularMoviesTitlesToFilter()
            }
        },
        ...
        sendPopularMoviesTitlesToFilter () {
            this.$eventBus.$emit('popular_movies_titles', this.titles)
        },
